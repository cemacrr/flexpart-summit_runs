...
OUT_DIR=$(dirname $(readlink -f ${0}))/multi
# job running script:
RUN_SCRIPT=$(dirname $(readlink -f ${0}))/../scripts/run_job.sh

# start time (2019-08-12 12:00):
START_TIME=1565607600
# end time (2019-08-07 12:00):
END_TIME=1565175600
...
do
  ...
  do
    # run dir:
    RD="${OUT_DIR}/${SD}${SH}-${ED}${EH}_REL_${REL_HEIGHT}_multi"
    ...
    # update outgrid file:
    sed -i "s|XXX|3,15,60,400,1000,1500,2000,3000|g" ${RD}/options/OUTGRID
    # update release file:
